...
engine = create_engine(
    f'mysql+mysqlconnector://root:{password}@localhost:3306/books',
    echo=True)
...
def add_book(book: Book, author: Author):
    with Session(engine) as session:
            ...
            if existing_book is not None:
                print("Books is already been added")
                return
            ...
            if existing_author is not None:
                print("Author has already added")
                session.flush()
            ...
            session.add(pairing)
            session.commit()
            print("new pairing added :{}".format(pairing))
