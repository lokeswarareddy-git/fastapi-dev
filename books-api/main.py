...
def create_book(request: schemas.BookAuthorPayload):
    database.add_book(convert_into_book_data_model(request.book), convert_into_author_data_model(request.author))
...
def convert_into_book_data_model(book: schemas.Book):
    return database.Book(title=book.title, number_of_pages=book.number_of_pages)


def convert_into_author_data_model(author: schemas.Author):
